...
<body>
    ...
    <br>
    <h3>
        <a href="/productSearch" class="right">Back to Product Search</a>
    </h3>
    <br>
        ...
        <c:if test="${cartItemsMapList.size() > 0}">
                ...
                <c:choose>
                    <c:when test="${cartItemsMap.key eq 'BOOK'}">
                        <table class="result" id="bookTable">
                            <tr>
                                <th colspan="8" class="center">Books</th>
                            </tr>
                            <tr>
                                <th>Product Id</th>
                                <th>Product Name</th>
                            ...
                            <c:forEach items="${cartItemsMap.value}" var="cartItem"
                                varStatus="status">
                                <tr>
                                    ...
                                    <td id="bookPrice${status.index}"><fmt:formatNumber
                                            type="number" maxFractionDigits="2"
                                            value="${cartItem.product.price}" /></td>
                                    <td id="bookTotalPrice${status.index}"><fmt:formatNumber
                                            type="number" maxFractionDigits="2"
                                            value="${cartItem.product.price * cartItem.quantity}" /></td>
                                    <td><input type="number" min="0"
                                        id="bookQuantity${status.index}"
                                        value='<c:out value="${cartItem.quantity}"/>'
                                        style="width: 10%; margin-right: 5px"> <input
                                        type="button" value="update"
                                        onclick="return updateCart(this, '${cartItem.product.id}', '${status.index}');">
                                        <input type="button" value="remove"
                                        onclick="return removeCartItem(this, '${cartItem.product.id}', '${status.index}');"></td>
                                </tr>
                            </c:forEach>
                        </table>
                    </c:when>
                    <c:when test="${cartItemsMap.key eq 'APPAREL'}">
                        <table class="result" id="apparelTable">
                            <tr>
                                <th colspan="8" class="center">Apparels</th>
                            </tr>
                            <tr>
                                <th>Product Id</th>
                                <th>Product Name</th>
                            ...
                            <c:forEach items="${cartItemsMap.value}" var="cartItem"
                                ...
                                <tr>
                                    ...
                                    <td id="apparelPrice${status.index}"><fmt:formatNumber
                                            type="number" maxFractionDigits="2"
                                            value="${cartItem.product.price}" /></td>
                                    <td id="apparelTotalPrice${status.index}"><fmt:formatNumber
                                            type="number" maxFractionDigits="2"
                                            value="${cartItem.product.price * cartItem.quantity}" /></td>
                                    <td><input type="number" min="0"
                                        id="apparelQuantity${status.index}"
                                        value='<c:out value="${cartItem.quantity}"/>'
                                        style="width: 10%; margin-right: 5px"> <input
                                        type="button" value="update"
                                        onclick="return updateCart(this, '${cartItem.product.id}', '${status.index}');">
            ...
            <table id="TotalAmountTable" class="result">
                <tr id="emptyCart" style="display: none;">
                    <th class="center">Cart is Empty!!!</th>
                </tr>
                <tr>
                    <th style="text-align: right; padding-right: 40px;">Total
                        Amount <span id="totalAmount"
