...
<body>
    ...
    <br>
    <h3>
        <a href="/viewCartDetails" class="right">View Items in Cart</a>
    </h3>
    <br>
    ...
    <div>
        <c:if test="${productsMap.size() > 0}">
            <c:forEach items="${productsMap}" var="productMap">
                <br>
                <c:choose>
                    <c:when test="${productMap.key eq 'BOOK'}">
                        <table class="result">
                            <tr>
                                <th colspan="7" class="center">Books</th>
                            </tr>
                            <tr>
                                <th>Product Id</th>
                                <th>Product Name</th>
                            ...
                            <c:forEach items="${productMap.value}" var="product">
                                <tr>
                                    ...
                                    <td><input type="number" min="1" value="1"
                                        ...
                                        onclick="return addToCart(this, '${product.id}');"></td>
                                </tr>
                            </c:forEach>
                        </table>
                    </c:when>
                    <c:when test="${productMap.key eq 'APPAREL'}">
                        <table class="result">
                            <tr>
                                <th colspan="7" class="center">Apparels</th>
                            </tr>
                            <tr>
                                <th>Product Id</th>
                                <th>Product Name</th>
        ...
        <c:if test="${productsMap.size() == 0}">
            <br>
            <h3 class="alert">No Matches found!!!</h3>
        </c:if>
    </div>
